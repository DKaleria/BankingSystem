...
public class AnalysisServiceImpl implements AnalysisService {
    ...
    private static final String INCOME_URL_TEMPLATE = "http://localhost:8082/incomes/%s/total?month=%d&year=%d";
    private static final String EXPENSE_URL_TEMPLATE = "http://localhost:8082/expenses/%s/total?month=%d&year=%d";
    ...
    @Override
    public BigDecimal getTotalBalance(String username, int month, int year) {
        System.out.println("📌 Запрос баланса для пользователя: " + username);

        BigDecimal totalIncome = getTotalIncome(username, month, year);
        BigDecimal totalExpense = getTotalExpense(username, month, year);

        return totalIncome.subtract(totalExpense); // ✅ Баланс = Доходы - Расходы
    }
    ...
    public BigDecimal getExpensePercentage(String username, int month, int year) {
        ...
        if (totalIncome.equals(BigDecimal.ZERO)) {
            return BigDecimal.ZERO; // 🔹 Если дохода нет, процент расходов = 0%
        }
