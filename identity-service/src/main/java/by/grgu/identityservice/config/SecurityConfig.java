    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf(CsrfConfigurer::disable)
                .authorizeHttpRequests(auth -> auth.requestMatchers(WHITELIST)
                        .permitAll()
                        .requestMatchers("/identity/users")
                        .hasAnyAuthority("ADMIN")
                        .anyRequest().authenticated())
                .addFilterBefore(jwtTokenFilter, UsernamePasswordAuthenticationFilter.class)
                .formLogin(login -> login.loginPage("/identity/login")
                        .defaultSuccessUrl("http://localhost:8082/accounts/account", true)
                        .permitAll()).logout(logout -> logout.logoutUrl("/identity/logout")
                        .logoutSuccessUrl("/identity/login").permitAll()).build();
    }*/
}
